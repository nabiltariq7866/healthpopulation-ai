...
function ConditionCohortTable({ type, rows, onOpen }: { type: keyof typeof config; rows: Patient[]; onOpen: (p: Patient) => void }) {
  ...
  return <div className="table-wrap"><table><thead><tr>{headers.map((header) => <th key={header}>{header}</th>)}</tr></thead><tbody>{rows.map((p) => {
    ...
    if (type === "diabetes") return <tr key={p.id}><td><b>{p.name}</b><small>{p.id}</small></td><td>{p.labs[0]?.result ?? "No recent result"}</td><td>{p.labs[0]?.trend ?? "Unknown"}</td><td>{p.medicationRisk ? "Potential concern" : "No current concern"}</td><td>{p.lastEncounter}</td><td>{openGap?.type ?? "No open gap"}</td><td><RiskTierBadge risk={p.risk} /></td><td><button className="secondary" onClick={() => onOpen(p)}>Review patient</button></td></tr>;
    if (type === "cardiovascular") return <tr key={p.id}><td><b>{p.name}</b><small>{p.id}</small></td><td>{p.screenings[0]?.type ?? "Cardiovascular review"}</td><td>{openGap?.due ?? p.screenings[0]?.date}</td><td>{p.lastEncounter}</td><td><CareGapBadge status={openGap?.status ?? "Completed"} /></td><td><RiskTierBadge risk={p.risk} /></td><td><button className="secondary" onClick={() => onOpen(p)}>Review patient</button></td></tr>;
    if (type === "hypertension") return <tr key={p.id}><td><b>{p.name}</b><small>{p.id}</small></td><td>{p.labs[0]?.result ?? "Synthetic observation"}</td><td>{p.labs[0]?.trend ?? "Review"}</td><td>{openGap?.due ?? "Current"}</td><td>{p.medications[0]?.refillIndicator ?? "Current"}</td><td><RiskTierBadge risk={p.risk} /></td><td><button className="secondary" onClick={() => onOpen(p)}>Review patient</button></td></tr>;
    if (type === "readmission") return <tr key={p.id}><td><b>{p.name}</b><small>{p.id}</small></td><td>{p.admissions}</td><td>{p.lastEncounter}</td><td>{p.gaps.find((g) => g.condition === "Readmission")?.status ?? "No open follow-up"}</td><td>{p.gaps.find((g) => g.type.includes("Medication"))?.status ?? "No current gap"}</td><td><RiskTierBadge risk={p.risk} /></td><td><button className="secondary" onClick={() => onOpen(p)}>Review patient</button></td></tr>;
    return <tr key={p.id}><td><b>{p.name}</b><small>{p.id}</small></td><td>{p.edVisits}</td><td>{p.admissions}</td><td>{p.lastEncounter}</td><td>{p.gaps.filter((g) => g.status !== "Completed").length}</td><td>{p.manager}</td><td><button className="secondary" onClick={() => onOpen(p)}>Review patient</button></td></tr>;
        ...
        <div className="grid two">
          {campaigns.map((c) => (
            <section className="card campaign" key={c.id}>
              <div className="card-head"><div><Badge tone={c.status === "Active" ? "green" : c.status === "Closed" ? "blue" : "gold"}>{c.status}</Badge><small>{c.id} · {c.channel}</small></div><span>{c.cohort}</span></div>
              <h3>{c.name}</h3>
